...
import {Breadcrumb, BreadcrumbItem, Button, Form, FormGroup, Label, Input, Call, Col, FormFeedback} from 'reactstrap';
...
class Contact extends Component {
    ...
    constructor(props){
        ...
        this.handleInputChange = this.handleInputChange.bind(this); //why??
    ...
    validate(firstname, lastname, telnum, email){
        ...
        const reg = /^\d+$/; //all numberss????
        if(this.state.touched.telnum && !reg.test(telnum))
            errors.telnum = 'Tel. Number should contain only numbers';

        if(this.state.touched.email && email.split('').field(x=> x==='0').length !== 1)
